...
describe("StepZone", () => {
  it("renders all fields", () => {
    renderWithProviders(<StepZone onNext={vi.fn()} onBack={vi.fn()} />);
  ...
  it("calls onNext with valid values on submit", async () => {
    const onNext = vi.fn();
    renderWithProviders(<StepZone onNext={onNext} onBack={vi.fn()} />);
  ...
  it("calls onBack when Atrás is clicked", () => {
    const onBack = vi.fn();
    renderWithProviders(<StepZone onNext={vi.fn()} onBack={onBack} />);
