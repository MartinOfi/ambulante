...
describe("LocationPermission", () => {
  ...
  it("shows 'Activar' button when status is prompt", () => {
    renderWithProviders(
      <LocationPermission status={LOCATION_PERMISSION_STATUS.PROMPT} onRequest={noop} />,
    );
  ...
  it("shows granted state text when permission is granted", () => {
    renderWithProviders(
      <LocationPermission status={LOCATION_PERMISSION_STATUS.GRANTED} onRequest={noop} />,
    );
  ...
  it("shows denied state message when permission is denied", () => {
    renderWithProviders(
      <LocationPermission status={LOCATION_PERMISSION_STATUS.DENIED} onRequest={noop} />,
    );
  ...
  it("shows unsupported message when API is unavailable", () => {
    renderWithProviders(
      <LocationPermission status={LOCATION_PERMISSION_STATUS.UNSUPPORTED} onRequest={noop} />,
    );
  ...
  it("calls onRequest when 'Activar' button is clicked", () => {
    ...
    renderWithProviders(
      <LocationPermission status={LOCATION_PERMISSION_STATUS.PROMPT} onRequest={onRequest} />,
    );
