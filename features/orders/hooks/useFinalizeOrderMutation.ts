...
export interface FinalizeOrderSuccess {
...
export function useFinalizeOrderMutation() {
  ...
  return useMutation({
    mutationFn: async (input: StoreOrderTransitionInput): Promise<FinalizeOrderSuccess> => {
      const session = await authService.getSession();
      if (session === null || session.user.role !== USER_ROLES.store) {
        logger.warn("useFinalizeOrderMutation: unauthorized finalize attempt", {
          publicId: input.publicId,
          role: session?.user.role ?? null,
        });
        throw new Error("Unauthorized: only store role can finalize orders");
      }
      const result = await finalizeOrder(input);
    ...
    onMutate: async (input: StoreOrderTransitionInput): Promise<MutateContext> => {
      ...
      queryClient.setQueryData<Order>(queryKeys.orders.byId(input.publicId), (old) => {
        ...
        return { ...old, status: ORDER_STATUS.FINALIZADO };
      });
    ...
    onError: (
      ...
      logger.error("useFinalizeOrderMutation: finalize failed", {
        publicId: input.publicId,
    ...
    onSettled: (
      _data: FinalizeOrderSuccess | undefined,
      _error: unknown,
      input: StoreOrderTransitionInput,
    ) => {
