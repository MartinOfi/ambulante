...
| Constants observability | `shared/constants/observability.ts` | `SLOW_QUERY_THRESHOLD_MS` (100ms), `SLOW_QUERY_BASELINE_FACTOR` (1.5×), `SLOW_QUERY_MAX_RESULTS` (50), `QUERY_TEXT_MAX_LENGTH` (500), `BASELINE_MIN_AGE_DAYS` (6). Constantes del cron `check-slow-queries` y de las funciones SQL `internal.get_slow_queries_for_alerts`/`snapshot_slow_query_baseline`. |
