...
| `getOrCreateRequestLogger` | `shared/utils/server-logger.ts` | `(source: Headers \| Request) => { logger: Logger; requestId: string }` — lee `x-request-id` del header (set por middleware) o genera uno nuevo, retorna child logger con binding. Usar como primer paso de un Route Handler / Server Action: `const { logger, requestId } = getOrCreateRequestLogger(request);` |
| `getRequestId` | `shared/utils/request-id.ts` | `(headers: Headers) => string \| null` — lee header `x-request-id` con trim; null si ausente o whitespace. |
| `readOrCreateRequestId` | `shared/utils/request-id.ts` | `(headers: Headers) => string` — usa el id entrante si es válido (no contiene `\r\n\0`, longitud ≤128), si no genera UUID v4. Defensa contra header-injection. Usado por middleware y `getOrCreateRequestLogger`. |
| `isValidRequestId` | `shared/utils/request-id.ts` | `(value: string) => boolean` — valida que el id no esté vacío, no exceda 128 chars y no contenga caracteres prohibidos (CR/LF/NUL). |
| `reportSlowQuery` | `shared/utils/sentry.ts` | `(payload: SlowQueryAlertPayload) => void` — envía un evento `level: "warning"` a Sentry con tags `breach_kind`/`request_id` y el payload completo en `extra`. Llamado por `/api/cron/check-slow-queries`. Tipo: `SlowQueryAlertPayload` (queryid, meanExecTimeMs, calls, totalExecTimeMs, queryText, baselineMeanMs, breachKind, requestId). |
| `REQUEST_ID_HEADER` | `shared/constants/observability.ts` | `"x-request-id"` — nombre canónico del header. Usar en lugar de strings literales en middleware/handlers. |
| Constants observability | `shared/constants/observability.ts` | `SLOW_QUERY_THRESHOLD_MS` (100ms), `SLOW_QUERY_BASELINE_FACTOR` (1.5×), `SLOW_QUERY_MAX_RESULTS` (50), `QUERY_TEXT_MAX_LENGTH` (500), `BASELINE_MIN_AGE_DAYS` (6). Constantes del cron `check-slow-queries`; las cuatro primeras se pasan como parámetros (`p_threshold_ms`, `p_limit`, `p_baseline_factor`, `p_baseline_min_age_days`) a `public.get_slow_queries_for_alerts`. La snapshot function vive en `internal.snapshot_slow_query_baseline()` (sólo cron, sin RPC). |
...
| `resizeImageForUpload` | `shared/utils/image-upload.ts` | `(file: File, options?: ResizeImageOptions) => Promise<ResizeImageResult>` — redimensiona client-side imágenes (jpeg/png/webp/gif) a `maxDimension` (default 1600px lado mayor) preservando aspect ratio. No-op si ya está dentro del límite o si el MIME no es redimensionable (ej. PDF). Defaults: `mimeType="image/jpeg"`, `quality=0.85` (clamp [0,1]). DI via `options.deps` (`createImageBitmap`, `createCanvas`) para tests. Retorna `{ file, didResize, originalDimensions, outputDimensions }`. Tipos exportados: `ResizeImageOptions`, `ResizeImageResult`, `ResizeImageDeps`, `CanvasLike`, `ImageDimensions`. Constantes: `DEFAULT_MAX_DIMENSION=1600`, `DEFAULT_OUTPUT_MIME="image/jpeg"`, `DEFAULT_QUALITY=0.85`, `RESIZABLE_IMAGE_TYPES`. |
