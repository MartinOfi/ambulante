...
- **API:** `ROUTES.auth.login`, `ROUTES.auth.register`, `ROUTES.auth.forgotPassword`, `ROUTES.auth.resetPassword`, `ROUTES.client.map`, `ROUTES.store.dashboard`, `buildHref(ROUTES.store.order, { orderId: "x" })`
