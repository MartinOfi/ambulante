...
- **Estado:** ✅ done [owner: chat-2026-04-29, finished: 12:00]
...
- **Estimación:** XL
- **Notas:** Scope real de B9-A limitado al prerequisito estructural del swap: (1) `shared/repositories/supabase/client.ts` — eliminados los throws a nivel de módulo; todas las factories ahora usan `getRequiredEnv()` dentro del cuerpo de función (seguras en import time). (2) `shared/repositories/index.ts` — factory pattern mock-or-Supabase idéntico al de `shared/services/index.ts`. Tests: servicios pasan en aislamiento con 1 failure pre-existente (`realtimeService.subscribe` aserción de TODO en `main`). Los swaps de features (auth/landing/map/store-detail) quedan como B9-B.
