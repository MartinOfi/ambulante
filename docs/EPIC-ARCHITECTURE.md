...
- **Depends on:** F15.5
- **Estimación:** L
- **Notas:** Elegido k6 (JS ecosystem, CLI simple, CI-friendly). 5 escenarios: `load-tests/scenarios/smoke.js` (1 VU/30s sanity), `store-discovery.js` (Q1 geoquery — p95<300ms), `order-flow.js` (ciclo completo §7.1 ENVIADO→FINALIZADO, custom metrics `order_acceptance_ms` < 3min y `order_finalization_rate` ≥70%), `location-update.js` (10→100 tiendas §7.5 cadencia 30-60s), `spike.js` (500 VUs race-condition §9.3, setup() siembra órdenes en RECIBIDO, expects 409 never 500). Config centralizada en `load-tests/k6.config.js`. ESLint ignorePatterns extendido a `load-tests/` (tooling externo, no app Next.js). Baselines TBD (requieren backend Supabase — F8). Docs en `docs/load-testing.md`.
